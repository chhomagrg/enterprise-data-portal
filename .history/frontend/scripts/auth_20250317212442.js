document.addEventListener("DOMContentLoaded", function () {
    ...
    async function checkAuthStatus() {  // Mark function as async
        const token = localStorage.getItem("token");
        ...
        if (token) {
            ...
            // Load user profile before initializing the calendar
            await loadUserProfile(token);
    
            // Add a delay to ensure everything is loaded before initializing the calendar
            setTimeout(() => {
                initializeCalendar();
            }, 500);
